...
def __getitem__(self, i: int) -> Dict:
    """
    Return a dictionary containing image tensor, class index, and disease label.
    ...
    Returns:
        dict: {
            "img": image tensor (after transforms),
            "cls": class index (int),
            "disease": disease label (int)
        }
    """
    f, j, fn, im = self.samples[i]  # filename, class index, npy filename, cached image
    if self.cache_ram:
        if im is None:  # load image if not cached
        ...
        im = cv2.imread(f)  # load image from file

    # Convert BGR (OpenCV) to RGB and then to PIL Image
    im = Image.fromarray(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))

    # Apply torch transforms (e.g. Resize, ToTensor, Normalize...)
    sample = self.torch_transforms(im)

    # Get disease label corresponding to index i
    disease_label = self.disease_labels[i]  # assuming it's a list or array of ints

    return {"img": sample, "cls": j, "disease": disease_label}
