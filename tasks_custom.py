...
from head_custom import Segment
...
class C2f(nn.Module):
    def __init__(self, c1, c2=None, n=1, shortcut=False, g=1, e=0.5):
        print(f"C2f.__init__ called with c1={c1}, c2={c2}")
        super().__init__()
        if c2 is None:
            c2 = c1
        self.c = int(c2 * e)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, layer in enumerate(d["backbone"] + d["head"]):
        if len(layer) == 5:
            f, n, m, args, kwargs = layer
        else:
            f, n, m, args = layer
            kwargs = {}

        m = (
            getattr(torch.nn, m[3:]) if isinstance(m, str) and "nn." in m
            else getattr(__import__("torchvision").ops, m[16:]) if isinstance(m, str) and "torchvision.ops." in m
            else globals()[m] if isinstance(m, str)
            else m
        )
        ...
        elif m in {Detect, Pose, OBB}:
            args.append([ch[x] for x in f])
            m.legacy = legacy

        if m is Segment:
            if isinstance(f, (list, tuple)):
                kwargs['ch'] = [flatten_ch(ch[x]) for x in f]
            else:
                kwargs['ch'] = [flatten_ch(ch[f])]
            if 'num_disease_classes' not in kwargs:
                kwargs['num_disease_classes'] = 7
            m.legacy = legacy
            c2 = kwargs['ch'][-1]
            if args:
                args = args[1:]
            ...
            if isinstance(f, int):
                ...
                c2 = get_int_ch(ch[f[-1]])
            if args and isinstance(args[0], int):
                if isinstance(f, int):
                    args[0] = get_int_ch(ch[f])
                else:
                    args[0] = max(get_int_ch(ch[x]) for x in f)

        m_ = nn.Sequential(*(m(*args, **kwargs) for _ in range(n_))) if n_ > 1 else m(*args, **kwargs)

        # Gán .f để YOLO biết layer lấy input từ đâu
        m_.f = f

        params = sum(p.numel() for p in m_.parameters())
        if verbose:
            LOGGER.info(f"{i:>3}{str(f):>20}{n_:>3}{params:10,d}  {m.__name__:<45}{str(args) + str(kwargs):<30}")
...
class YOLOSegResNetModel(SegmentationModel):
    def __init__(self, cfg='your_custom.yaml', ch=3, nc=None, verbose=True, num_disease_classes=5):
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
        self.num_disease_classes = num_disease_classes

        from torchvision.models import resnet18
        self.disease_head = resnet18(pretrained=True)
        self.disease_head.fc = nn.Linear(self.disease_head.fc.in_features, num_disease_classes)

        self.ce_loss = nn.CrossEntropyLoss()
        self.criterion = None

    def forward(self, x):
        # Thay vì gọi super().forward(x) vì trả về list sẽ gây lỗi conv2d
        # gọi super().predict(x) để lấy tensor mask, boxes, classes...
        yolo_out = super().predict(x)  

        # Lấy feature cuối từ backbone
        feat = self.backbone(x)[-1]
        pooled = feat.mean(dim=[2, 3])

        # Dự đoán disease class
        disease_out = self.disease_head(pooled)

        # Trả về tuple tất cả outputs
        return (*yolo_out, disease_out)
    ...
    def loss(self, batch, preds=None):
        ...
        preds = self.forward(batch["img"]) if preds is None else preds
        yolo_preds = preds[:-1]
        disease_preds = preds[-1]

        loss_dict = self.criterion(yolo_preds, batch)

        target_disease = batch.get("disease_ids")
        target_disease = target_disease.view(-1).long().to(disease_preds.device)

        disease_preds = disease_preds.view(-1, self.num_disease_classes)

        disease_loss = self.ce_loss(disease_preds, target_disease)
        loss_dict["loss_disease"] = disease_loss
        loss_dict["loss"] += disease_loss * 1.0  # tùy chỉnh lambda nếu cần
