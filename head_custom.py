...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=None, ch=(), inplace=True, num_disease_classes=7, **kwargs):
        super().__init__()
        ...
        self.disease_branch = models.resnet18(pretrained=True)
        self.disease_branch.fc = nn.Linear(512, num_disease_classes)
...
class Segment(Detect):
    def __init__(self, nc=80, nm=32, npr=256, ch=(), num_disease_classes=7, *args, **kwargs):
        super().__init__(nc=nc, ch=ch, *args, **kwargs)  # gọi init của Detect
    ...
    def forward(self, x: List[torch.Tensor]) -> Union[Tuple, List[torch.Tensor]]:
        """
        Args:
            x (List[Tensor]): Feature maps từ backbone
        
        Returns:
            Nếu training: (yolo_outputs, mask_coefficients, protos, disease_outputs)
            Nếu eval: (combined_outputs, protos, disease_outputs)
        """
        p = self.proto(x[0])  # mask prototypes
        bs = p.shape[0]  # batch size

        mc = torch.cat([self.cv4[i](x[i]).view(bs, self.nm, -1) for i in range(len(self.cv4))], 2)  # mask coefficients

        # Giả sử Detect.forward là hàm forward của head detect, bạn thay thế đúng theo repo
        # Ví dụ:
        yolo_outputs = Detect.forward(self, x)  

        # Lấy feature cuối cùng để đưa vào ResNet (bạn có thể thay đổi pooling tùy ý)
        pooled = x[-1].mean(dim=[2, 3])  # Global Average Pooling

        disease_outputs = self.disease_branch(pooled)  # dự đoán disease_id

        if self.training:
            return yolo_outputs, mc, p, disease_outputs
        else:
            # Khi inference, kết hợp yolo và mask coeffs
            combined = torch.cat([yolo_outputs[0], mc], 1) if isinstance(yolo_outputs, tuple) else torch.cat([yolo_outputs, mc], 1)
            if hasattr(self, 'export') and self.export:
                return combined, p, disease_outputs
            return (combined, p), disease_outputs
